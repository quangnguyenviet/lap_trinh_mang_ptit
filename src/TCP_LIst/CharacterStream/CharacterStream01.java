/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */

package TCP_LIst.CharacterStream;

/**
 *
 * @author nvqua
 */
//[Mã câu hỏi (qCode): h2iSiuQr].  Một chương trình server cho phép kết nối qua giao thức TCP tại cổng 2208 (hỗ trợ thời gian giao tiếp tối đa cho mỗi yêu cầu là 5s). Yêu cầu là xây dựng một chương trình client tương tác với server sử dụng các luồng byte (BufferedWriter/BufferedReader) theo kịch bản sau: 
//a.	Gửi một chuỗi gồm mã sinh viên và mã câu hỏi với định dạng "studentCode;qCode". Ví dụ: "B15DCCN999;EC4F899B"
//b.	Nhận một chuỗi ngẫu nhiên là danh sách các một số tên miền từ server
//Ví dụ: giHgWHwkLf0Rd0.io, I7jpjuRw13D.io, wXf6GP3KP.vn, MdpIzhxDVtTFTF.edu, TUHuMfn25chmw.vn, HHjE9.com, 4hJld2m2yiweto.vn, y2L4SQwH.vn, s2aUrZGdzS.com, 4hXfJe9giAA.edu
//c.	Tìm kiếm các tên miền .edu và gửi lên server
//Ví dụ: MdpIzhxDVtTFTF.edu, 4hXfJe9giAA.edu
//d.	Đóng kết nối và kết thúc chương trình.
import java.util.*;
import java.io.*;
import java.net.*;
public class CharacterStream01 {
    public static void main(String[] args) {
        try {
            String host = "203.162.10.109";
            int port = 2208;
            Socket sk = new Socket(host, port);
            String s1 = "B22DCCN650;h2iSiuQr";
            BufferedReader in = new BufferedReader(new InputStreamReader(sk.getInputStream()));
            BufferedWriter out = new BufferedWriter(new OutputStreamWriter(sk.getOutputStream()));
            out.write(s1);
            out.newLine();
            out.flush();
            
            String s = in.readLine();
            
            List<String> l = new ArrayList<>(
                    Arrays.asList(
                            s.trim().split(", ")
                    )
            );
            List<String> l1 = new ArrayList<>();
            for(String tmp : l){
                if(tmp.endsWith(".edu")) l1.add(tmp);
            }
            
            String s2 = String.join(", ", l1);
            System.out.println("result " + s2);
            out.write(s2);
            out.newLine();
            out.flush();
            
            out.close();
            in.close();
            sk.close();
            
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
    }
}
